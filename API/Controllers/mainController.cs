...
namespace EStore.Controllers
{
    ...
    public class mainController : baseApiContoller
    {
        ...
        public mainController(AppDbContext context, UserManager<user> user)
        {
            _context = context;
            _user = user;
        }
        ...
        public async Task<ActionResult<PagedList<item>>> Get_All_Items(
        ...
        )
        {
            ...
            var _query = _context.ITEMS
                ...
                .AsQueryable();
            var items = await PagedList<item>.ToPagedList(
                _query,
        ...
        public async Task<ActionResult> Get_Item_By_Id(int Item_Id, int Company_Id)
        {
            var items = await _context.ITEMS
                ...
                .Where(o => o.Id == Item_Id && o.Company_Id == Company_Id)
                .FirstOrDefaultAsync();
            return Ok(items);
        }
        ...
        {
            var _items = await _context.ITEM_DETAILS.Where(o => o.Item_Id == item_Id).ToListAsync();
            var _items2 = _items
                .Select(
                    o =>
                        new
        ...
        public async Task<IActionResult> GetUniqueFilters(int company_Id)
        {
            ...
            var _item = await _context.ITEMS
                .Where(o => o.Company_Id == company_Id && o.Is_Active == 1)
                .ToListAsync();
            var _item_Details = await _context.ITEM_DETAILS
                .Where(u => _item.Select(o => o.Id).ToList().Contains(u.Item_Id))
                .ToListAsync();
            var _company_Details = await _context.COMPANY
                .Where(i => i.Id == company_Id)
                .FirstOrDefaultAsync();
        ...
        {
            var _companies = await _context.COMPANY
                .Select(
                    o =>
                        new
                        {
